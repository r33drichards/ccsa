...
// ---- Sorting arena ---------------------------------------------------------
// A different task shape: sort the items in each ADJACENT chest IN PLACE —
// consolidate same items into minimal stacks and order slots by item name,
// without moving items between chests. Checked by invariants: conservation,
// consolidation (#slots == ceil(count/64)), and sorted-by-name.
type Stack = [string, number];
const SORT_DIRS: Record<string, [number, number, number]> = {
  up: [8, 65, 8], down: [8, 63, 8], front: [8, 64, 9],   // turtle at 8,64,8 facing south
};

function mergedCounts(stacks: Stack[]): Record<string, number> {
  const m: Record<string, number> = {};
  for (const [n, c] of stacks) m[n] = (m[n] || 0) + c;
  return m;
}
function luaExpected(m: Record<string, number>): string {
  return "{ " + Object.entries(m).map(([n, c]) => `['${n}'] = ${c}`).join(", ") + " }";
}
function luaStacks(stacks: Stack[]): string {
...
function genSortSpec(): { yaml: string } {
  ...
  const envs: { note: string; chests: Record<string, Stack[]> }[] = [
    ...
    {
      note: "single-item fragmentation, distinct singles, and an empty chest",
      chests: {
          ...
          test = function(sim)
            local function check(x, y, z, expected)
              ...
              for name, c in pairs(expected) do
                sim.assertEq(counts[name] or 0, c, name .. ' count preserved')
                sim.assertEq(slots[name] or 0, math.ceil(c / 64), name .. ' consolidated to minimal stacks')
              end
              for name, c in pairs(counts) do sim.assertEq(expected[name] or 0, c, 'no stray ' .. name) end
              sim.assertTrue(sorted, 'slots sorted by item name @' .. x .. ',' .. y .. ',' .. z)
            end
${checks}
          end,
        }`;
  ...
  const yaml = `# Generated by create_sort_sim — the arena for an in-place item sorter.
# Sort each adjacent chest (up, down, front) in place: consolidate + order by name.
task: >-
  Stationary turtle. For EACH adjacent chest (above, below, and in front), sort its
  contents IN PLACE: merge same items into the fewest stacks and order the slots by
  item name. Do NOT move items between chests and do NOT lose or duplicate anything.
  Handle empty chests gracefully.
sim:
  timeout_ms: 60000
  nodes:
${envs.map((e, i) => node(i + 1, e)).join("\n")}
...
export default function piTurtle(pi: ExtensionAPI) {
  (pi.registerTool as (t: unknown) => unknown)({
    name: "create_sort_sim",
    label: "Create Sort Sim",
    description:
      "Build the ARENA for an in-place item-SORTER turtle (a different task from compression). " +
      "It writes environments with adjacent chests (above, below, in front) full of fragmented, " +
      "unsorted items, checked with invariants: conservation (nothing lost/duplicated), " +
      "consolidation (same items merged into minimal stacks), and slots ordered by item name. " +
      "Takes no arguments. Call this FIRST for a sorting turtle, then iterate with turtle_sim.",
    promptSnippet: "Build the arena for an in-place item-sorter turtle (adjacent chests).",
